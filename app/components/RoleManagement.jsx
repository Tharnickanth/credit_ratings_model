...
import { Shield, Save, X, Loader2, Plus, Edit2 } from 'lucide-react';
...
const RoleManagement = () => {
  ...
  const logActivity = async (description, action = 'Role Management') => {
    if (!currentUser) {
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      if (isEditing) {
        await logActivity(
          `Updated role: ${formData.roleName} (${formData.permissions.length} permissions)`,
          'Role Update'
        );
      } else {
        await logActivity(
          `Created new role: ${formData.roleName} (${formData.permissions.length} permissions)`,
          'Role Creation'
        );
      }
  ...
  const handleEdit = (role) => {
    ...
    // Scroll to the role name input and focus it
    setTimeout(() => {
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      {/* EXISTING ROLES SECTION - Card Grid Format */}
      <div className="bg-white rounded-lg shadow">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-gray-800">Existing Roles</h2>
              <p className="text-sm text-gray-600 mt-1">View and manage existing roles</p>
            </div>
            <button
              onClick={() => {
                handleReset();
        ...
        <div className="p-6">
          {isFetchingRoles ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">No roles found. Create your first role below.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {roles.map((role) => (
                ...
                >
                  ...
                  {/* Role Name */}
                  <h3 className="font-semibold text-gray-900 text-lg mb-1 truncate" title={role.roleName}>
                    {role.roleName}
                  </h3>
                  ...
                  <div className="pt-3 border-t border-gray-200">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 px-3 py-2 text-cyan-600 hover:bg-cyan-50 rounded-lg transition-colors"
                      title="Edit role"
                    >
                      <Edit2 size={16} />
      ...
      <ConfirmationModal
        ...
        title={isEditing ? 'Confirm Role Update' : 'Confirm Role Creation'}
        message={
          isEditing
            ? `Are you sure you want to update the role "${formData.roleName}" with ${formData.permissions.length} permissions?`
            : `Are you sure you want to create a new role "${formData.roleName}" with ${formData.permissions.length} permissions?`
        }
        confirmText={isEditing ? 'Update Role' : 'Create Role'}
        cancelText="Cancel"
